...
class xPacketBuffer final : xNonCopyable {
public:
...
public:
	...
	X_INLINE bool HasNext() const {
		return NextBufferPtr;
	}
...
class xPacketBufferChain final : xNonCopyable {
public:
	...
	X_INLINE size_t GetSize() const {
		return _TotalBufferCount;
	}
	X_INLINE bool IsEmpty() const {
		return !GetSize();
	}
	X_INLINE xPacketBuffer * GetLast() const {
		return _LastPtr;
	}
