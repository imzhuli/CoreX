...
public:
	...
	struct iListener {
		 ...
		 * but to do that, it bacome also very important to prevent callback reentry issue, like OnFlush->PostData->OnFlush, which is
		 * rere in most cases. So OnFlush is only called when an asynchronous call is done.
		 *
		 * Practise:
		 * If some just wants to send Data immediately when connection is established, just call PostData after Connection->Init, data
		 * is buffered, and sent when connection is established But in case establish connection took some long time, and data is quite
		 * droppable, check connection state by IsConnected() after calling init, and decide to PostData or not
		 *
		...
		X_API_MEMBER virtual size_t OnData(xTcpConnection * TcpConnectionPtr, void * DataPtr, size_t DataSize) {
			return DataSize;
		}
		...
		X_API_MEMBER virtual void OnError(xTcpConnection * TcpConnectionPtr) {
			OnPeerClose(TcpConnectionPtr);
		}
	};
...
public:
	...
	X_INLINE bool IsConnected() const {
		return _Status == eStatus::Connected;
	}
	X_INLINE bool HasPendingWrites() const {
		return _HasPendingWriteFlag;
	}
	X_INLINE void SetMaxWriteBufferSize(size_t NewLimit) {
		_MaxWriteBufferSize = NewLimit;
	}
...
private:
	...
	size_t _MaxWriteBufferSize = SIZE_MAX / 2;
};
