...
struct xNetAddress final {
	...
	X_INLINE bool operator!=(const xNetAddress & Other) const { return !(*this == Other); }
	...
	X_STATIC_INLINE xNetAddress		Parse(const struct sockaddr_storage * SockAddrStoragePtr) { return Parse((sockaddr *)SockAddrStoragePtr); }
};
