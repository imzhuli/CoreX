...
bool xTcpServer::Init(xIoContext * IoContextPtr, const xNetAddress & Address, iListener * ListenerPtr, bool ReusePort) {
	...
	if (!(_IoBufferPtr = CreateOverlappedObject())) {
		return false;
	}
...
void xTcpServer::TryPreAccept() {
	...
	auto AcceptResult = AcceptEx(
		_ListenSocket, _PreAcceptSocket, &_PreAcceptAddress, 0, sizeof(_PreAcceptAddress.Local) + 16, sizeof(_PreAcceptAddress.Remote) + 16,
		&_PreAcceptAddress._PreAcceptReceivedLength, &Overlapped
	);
