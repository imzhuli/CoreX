...
bool xIoContext::Init() {
	...
	auto PollerGuard = xScopeGuard([&] { CloseHandle(X_DEBUG_STEAL(_Poller, InvalidEventPoller)); });
...
void xIoContext::LoopOnce(int TimeoutMS) {
	...
	BOOL             Result     = GetQueuedCompletionStatusEx(
        _Poller, EventEntries, (ULONG)Length(EventEntries), &EventCount, (TimeoutMS < 0 ? INFINITE : (DWORD)TimeoutMS), FALSE
    );
