...
public:
	virtual void OnIoEventInReady() {
		Pass();
	}
	virtual void OnIoEventOutReady() {
		Pass();
	}
	virtual void OnIoEventError() {
		Pass();
	}

	X_PRIVATE_INLINE bool IsAvailable() const {
		return !_StatusFlags;
	}
	X_PRIVATE_INLINE bool IsDisabled() const {
		return _StatusFlags & SF_Disabled;
	}
	X_PRIVATE_INLINE bool HasError() const {
		return _StatusFlags & SF_Error;
	}
...
protected:
	X_INLINE void SetAvailable() {
		_StatusFlags = 0;
	}
	X_INLINE void SetEnabled() {
		_StatusFlags &= ~SF_Disabled;
	}
	X_INLINE void SetDisabled() {
		_StatusFlags |= SF_Disabled;
	}
	X_INLINE void ClearError() {
		_StatusFlags &= ~SF_Error;
	}
	X_INLINE void SetError() {
		_StatusFlags |= SF_Error;
	}
};

class xIoContext : xNonCopyable {
public:
...
private:
	xEventPoller        _Poller              = InvalidEventPoller;
...
protected:
	...
	X_INLINE void SetReadTransfered(DWORD Size) {
		_IoBufferPtr->ReadObject.DataSize = Size;
	}
	X_INLINE void SetWriteTransfered(DWORD Size) {
		_IoBufferPtr->WriteObject.DataSize = Size;
	}

protected:
	struct xOverlappedIoBuffer;
	struct xOverlappedObject : private ::xel::xNonCopyable {
	...
	struct xOverlappedIoBuffer : ::xel::xNonCopyable {
		bool               DeleteMark;
	...
	X_PRIVATE_STATIC_MEMBER void                  RetainOverlappedObject(xOverlappedIoBuffer * IoBufferPtr) {
        ++IoBufferPtr->ReferenceCount;
	}
