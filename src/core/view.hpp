...
public:
	...
	X_INLINE xDataView(const T (&refArray)[N]) : xDataView(&refArray, sizeof(refArray)) {}

	template <size_t N>
	X_INLINE explicit xDataView(const xDataView (&refArray)[N]) : xDataView(&refArray, sizeof(refArray)) {}
	...
	X_INLINE const ITEM_TYPE * data() const {
		static_assert(!std::is_reference_v<ITEM_TYPE>);
		return static_cast<const ITEM_TYPE *>(pData);
	}
	X_INLINE const ubyte * data() const { return static_cast<const ubyte *>(pData); }

	template <typename SIZE_TYPE>
	X_INLINE SIZE_TYPE size() const {
...
public:
	...
	X_INLINE xRangeView(T * start, T * end) : _Start(start), _End(end), _Size(end - start) { assert(_Start <= _End); }

	template <typename N, typename = std::enable_if_t<std::is_integral_v<N> && !std::is_pointer_v<N>>>
	X_INLINE xRangeView(T * start, N number) : _Start(start)
											 , _End(start + number)
											 , _Size(number) {
		assert(_Start <= _End);
	}
	...
	template <size_t N>
	xRangeView(T (&start)[N]) : _Start(start)
							  , _End(start + N)
							  , _Size(N) {}
...
public:
	...
	public:
		X_INLINE xIterator(IteratorInitPointer ptr, size_t stride)
			: _Memory(reinterpret_cast<IteratorMemory>(ptr))
			, _Stride(stride) {}
